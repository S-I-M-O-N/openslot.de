...
ISR (INT0_vect) {
  GICR &= ~_BV(INT0); // temporarily disable INT0 interrupts
    // car0 detector
//    uint16_t tmp = 0;
...
//        if ( (tmp > 54) && (tmp < 74) ) car0 = 1;
		  //if (car0 == 0) car0 = 1;
  	  	  if (mode == 2) {
  	  		  mode = 0xFF;
  	  		  slot[0].jumpstart_time = sysclk.word[0]; // low word
  	          RS232_putc('~');
	          RS232_putc('1');
  	  		  RS232_putc(':');
...
ISR (INT1_vect) {
    GICR &= ~_BV(INT1); // temporarily disable INT1 interrupts
    // car1 detector
//    uint16_t tmp = 0;
...
//        if ( (tmp > 118) && (tmp < 138) ) car1 = 2;
	      //if (car1 == 0) car1 = 2;
  	  	  if (mode == 2) {
  	  		  mode = 0xFF;
  	  		  slot[1].jumpstart_time = sysclk.word[0]; // low word
  	  		  RS232_putc('~');
  	  		  RS232_putc('2');
  	  		  RS232_putc(':');
